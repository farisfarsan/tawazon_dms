...
# ---------- MANUAL BACKUP (admin-only: exports every client's data) ----------

# Regenerated by Django itself on every deploy/migrate, or genuinely not
# something you want reloaded from an old backup — leaving them out keeps
# the dump restorable against a codebase that has since gained/lost a
# permission or a live session, rather than fighting a stale one.
BACKUP_EXCLUDED_APPS = ['contenttypes', 'auth.permission', 'admin.logentry', 'sessions.session']


@admin_required
def backup_settings(request):
    return render(request, 'settings_backup.html', {
        'active_page': 'settings_backup',
        'active_sub':  'settings_backup',
        'attachment_count': CaseAttachment.objects.count(),
    })


@admin_required
def backup_download_db(request):
    """A full JSON dump of every table (Django's own dumpdata) — restorable
    with `manage.py loaddata` into a fresh database of the same app version.
    Built in memory and streamed straight to the browser; nothing is written
    to the container's own (ephemeral) disk."""
    import io
    from django.core.management import call_command
    from django.utils import timezone

    buf = io.StringIO()
    call_command('dumpdata', exclude=BACKUP_EXCLUDED_APPS, indent=2, stdout=buf)

    filename = f'tawazon-db-backup-{timezone.now():%Y-%m-%d-%H%M%S}.json'
...
def backup_download_media(request):
    ...
    dev), zipped on the fly — nothing is written to the container's own
    disk either, the zip is assembled straight into the response.

    This is a single synchronous request: with a great many attachments it
    can run long enough to hit gunicorn's worker timeout (WEB_TIMEOUT, 60s
    by default). If that starts happening, raise WEB_TIMEOUT in Railway's
    variables rather than changing this — there's no partial-download
    story worth building for a manual, occasional export."""
    import zipfile
    ...
    with zipfile.ZipFile(buf, 'w', zipfile.ZIP_DEFLATED) as zf:
        for att in CaseAttachment.objects.select_related('case').iterator():
            if not att.file:
                continue
            try:
                ...
                skipped.append(f'{att.case.case_id if att.case_id else "?"} / {att.filename or att.file.name}: {e}')
                continue
            case_ref = att.case.case_id if att.case_id else 'unfiled'
            arcname = f'{case_ref}/{att.filename or att.file.name.rsplit("/", 1)[-1]}'
            zf.writestr(arcname, data)
        if skipped:
            zf.writestr('_skipped_files.txt',
                         'These files could not be read from storage and were left out:\n\n' + '\n'.join(skipped))

    filename = f'tawazon-attachments-{timezone.now():%Y-%m-%d-%H%M%S}.zip'
